...
"Chroma Corner" is the name of a new technique to create rounded corners in IE 6 - 8. It uses IE's chroma-filter as a tool to mask away corner areas, which are created as a graphic and which are filled with a solid color. 
...
_*Important:* Please note that you need to set the url of the behavior *not* relative to the stylesheet but relative to the HTML-document. The best thing is to use an absolute path like behavior: url(/css/chromacorners/chromacorners.htc)._ 
...
_*Note:* This is planned to get fixed in a future version._
